...
public class AdminArticleServiceImpl implements AdminArticleService {
    ...
    private String convertUrl(String url) {
        ...
        if (url.contains(apiEndpoint)) {
            ...
            log.debug("转换Minio URL: {} -> {}", url, convertedUrl);
            return convertedUrl;
        }
    ...
    public Response publishArticle(PublishArticleReqVO publishArticleReqVO) {
        boolean isExecuteSuccess = transactionTemplate.execute(status -> {
            // 转换标题图片URL
            String titleImage = publishArticleReqVO.getTitleImage();
            if (titleImage != null && !titleImage.isEmpty()) {
                titleImage = convertUrl(titleImage);
                publishArticleReqVO.setTitleImage(titleImage);
            }
    ...
    public Response queryArticleDetail(QueryArticleDetailReqVO queryArticleDetailReqVO) {
        ...
        // 转换图片URL
        String titleImage = articleDO.getTitleImage();
        if (titleImage != null && !titleImage.isEmpty()) {
            titleImage = convertUrl(titleImage);
        }
    ...
    public Response queryArticlePageList(QueryArticlePageListReqVO queryArticlePageListReqVO) {
        ...
        if (records != null) {
            for (ArticleDO article : records) {
                if (article.getTitleImage() != null && !article.getTitleImage().isEmpty()) {
                    article.setTitleImage(convertUrl(article.getTitleImage()));
                }
            }
        }
    ...
    public Response updateArticle(UpdateArticleReqVO updateArticleReqVO) {
        boolean isExecuteSuccess = transactionTemplate.execute(status -> {
            ...
            // 转换标题图片URL
            String titleImage = updateArticleReqVO.getTitleImage();
            if (titleImage != null && !titleImage.isEmpty()) {
                titleImage = convertUrl(titleImage);
                updateArticleReqVO.setTitleImage(titleImage);
            }
