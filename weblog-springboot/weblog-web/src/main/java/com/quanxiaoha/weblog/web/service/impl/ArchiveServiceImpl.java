    ...
    public Response queryArchive(QueryArchivePageListReqVO queryArchivePageListReqVO) {
        ...
        if (!CollectionUtils.isEmpty(records)) {
            itemRspVOList = records.stream()
                    .map(articleDO -> {
                        QueryArchiveItemRspVO itemRspVO = articleConvert.convert2Archive(articleDO);
                        // 转换文章封面图URL
                        if (itemRspVO.getTitleImage() != null && !itemRspVO.getTitleImage().isEmpty()) {
                            itemRspVO.setTitleImage(minioUrlConverter.convert(itemRspVO.getTitleImage()));
                        }
                        return itemRspVO;
                    })
                    .collect(Collectors.toList());

            Map<String, List<QueryArchiveItemRspVO>> map = itemRspVOList.stream()
                    .collect(Collectors.groupingBy(QueryArchiveItemRspVO::getCreateMonth));
